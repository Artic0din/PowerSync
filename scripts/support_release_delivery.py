...
class ReleaseDelivery:
    ...
    def record(self, repository: str, release: dict[str, Any]) -> str:
        ...
        for pull_request in pull_requests:
            issue_numbers = self._referenced_issues(
                repository, pull_request.get("body", "")
            )
            ...
            for issue_number in issue_numbers:
                deliveries.setdefault(issue_number, pull_url)

        recorded = sum(
            self._record_issue(repository, issue_number, pull_url, release_url)
            for issue_number, pull_url in deliveries.items()
        )
