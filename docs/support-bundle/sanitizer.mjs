...
async function digest(value) {
  ...
  return Array.from(new Uint8Array(hash).slice(0, 5), (byte) => byte.toString(16).padStart(2, "0")).join("");
}
...
async function pseudonymise(text, label, pattern) {
  ...
  for (const value of values) {
    replacements.set(value, `[${label}_${await digest(value)}]`);
  }
...
function downloadBundle(content, originalName) {
  const safeName = originalName.replace(/\.[^.]+$/, "").replace(/[^A-Za-z0-9_-]+/g, "-");
  const link = document.createElement("a");
  link.href = URL.createObjectURL(new Blob([content], { type: "text/plain;charset=utf-8" }));
  link.download = `${safeName || "powersync"}-sanitised.log`;
  link.click();
...
if (typeof document !== "undefined") {
  ...
  createButton.addEventListener("click", async () => {
    ...
    try {
      ...
      downloadBundle(content, file.name);
      status.textContent = "Bundle created. Review the downloaded file before uploading it.";
    } catch {
      status.textContent = "The bundle could not be created. Use a UTF-8 text export and try again.";
    } finally {
